...
public class ChannelsWriterTest {
    ...
    public void wrappedBuffer() throws Exception {
        ...
        assertThat(out.toByteArray()).hasSize(3).isEqualTo(new byte[] {1, 2, 3});

        assertThat(isFlush).hasSize(1).containsOnly(true);

        assertThat(((Buffer) bb).position()).isEqualTo(3);

        assertThat(((Buffer) bb).limit()).isEqualTo(3);

        assertThat(bb.capacity()).isEqualTo(3);
    ...
    public void bigBuffer() throws Exception {
        ...
        assertThat(bb.capacity()).isEqualTo(4);
    ...
    public void shouldFlushWhenEmptyBuffer() throws Exception {
        ...
        int countWritten = channel.write(bb);
        assertThat(countWritten).isEqualTo(0);
    ...
    public void bufferedChannel() throws Exception {
        ...
        assertThat(out.toByteArray()).hasSize(4).isEqualTo(new byte[] {1, 2, 3, 4});

        assertThat(((Buffer) bb).position()).isEqualTo(3);

        assertThat(((Buffer) bb).limit()).isEqualTo(3);

        assertThat(bb.capacity()).isEqualTo(5);
    ...
    public void shouldFailIOnDirectBuffer() throws IOException {
        ByteArrayOutputStream out = new ByteArrayOutputStream();
        WritableByteChannel channel = Channels.newChannel(out);
        assertThrows(NonWritableChannelException.class, () -> channel.write(ByteBuffer.allocateDirect(0)));
    }

    @Test
    public void shouldUseFileChannel() throws IOException {
        File f = createFile(tmp.resolve("test")).toFile();
        ...
        assertThat(readAllBytes(f.toPath())).hasSize(3).isEqualTo(new byte[] {1, 2, 3});
    }
    ...
    public void shouldWriteArray() throws Exception {
        AsynchronousByteChannel channel = mock(AsynchronousByteChannel.class);
        when(channel.write(any(ByteBuffer.class))).thenAnswer(new Answer<Future<Integer>>() {
            @Override
        ...
        assertThat(captured.getAllValues()).hasSize(1);

        assertThat(captured.getAllValues().get(0).array()).containsOnly(new byte[] {1, 2, 3, 4, 5});

        assertThat(captured.getAllValues().get(0).arrayOffset()).isEqualTo(0);

        assertThat(captured.getAllValues().get(0).position()).isEqualTo(4);

        assertThat(captured.getAllValues().get(0).limit()).isEqualTo(4);

        assertThat(captured.getAllValues().get(0).capacity()).isEqualTo(5);
    }
    ...
    public void shouldWrite() throws Exception {
        ...
        assertThat(captured.getAllValues().get(0).array()).containsOnly(new byte[] {3});

        assertThat(captured.getAllValues().get(0).arrayOffset()).isEqualTo(0);

        assertThat(captured.getAllValues().get(0).position()).isEqualTo(1);

        assertThat(captured.getAllValues().get(0).limit()).isEqualTo(1);

        assertThat(captured.getAllValues().get(0).capacity()).isEqualTo(1);
    }

    @Test
    public void shouldThrowExceptionOnWrite() throws Exception {
        AsynchronousByteChannel channel = mock(AsynchronousByteChannel.class);
        when(channel.write(any(ByteBuffer.class))).thenThrow(ShutdownChannelGroupException.class);
        OutputStream os = Channels.newOutputStream(channel);
        IOException ex = assertThrows(IOException.class, () -> os.write(new byte[1], 0, 1));
    ...
    public void shouldThrowExceptionOnFuture1() throws Exception {
        ...
        when(channel.write(any(ByteBuffer.class))).thenReturn(future);
        OutputStream os = Channels.newOutputStream(channel);
        assertThrows(InterruptedIOException.class, () -> os.write(new byte[1], 0, 1));
    }
