...
public class Surefire1535TestNGParallelSuitesIT extends SurefireJUnitIntegrationTestCase {
    ...
    public void forks2() throws VerificationException {
        OutputValidator validator = unpack().activateProfile("forked-reports-directory")
                .forkCount(2)
                .executeTest();
        ...
        assertThat(parallelTest11 ^ parallelTest12).isTrue();
        ...
        assertThat(parallelTest21 ^ parallelTest22).isTrue();

        assertThat(parallelTest11 && parallelTest22 || parallelTest12 && parallelTest21)
                .isTrue();

        validator
                .assertThatLogLine(containsString(TEST_RESULT_2), is(1))
    ...
    public void forks2Redirected() throws VerificationException {
        ...
        boolean parallelTest12 = xml.contains("<testcase name=\"test\" classname=\"it.ParallelTest2\"");
        assertThat(parallelTest11 ^ parallelTest12).isTrue();
        String log = validator
                .getSurefireReportsFile("../surefire-reports-1/TestSuite-output.txt", UTF_8)
                .readFileToString();
        assertThat(log.contains(TEST1)).isEqualTo(parallelTest11);
        assertThat(log.contains(SUITE1)).isEqualTo(parallelTest11);
        assertThat(log.contains(TEST2)).isEqualTo(parallelTest12);
        assertThat(log.contains(SUITE2)).isEqualTo(parallelTest12);
        ...
        assertThat(parallelTest21 ^ parallelTest22).isTrue();
        log = validator
                .getSurefireReportsFile("../surefire-reports-2/TestSuite-output.txt", UTF_8)
                .readFileToString();
        assertThat(log.contains(TEST1)).isEqualTo(parallelTest21);
        assertThat(log.contains(SUITE1)).isEqualTo(parallelTest21);
        assertThat(log.contains(TEST2)).isEqualTo(parallelTest22);
        assertThat(log.contains(SUITE2)).isEqualTo(parallelTest22);
    ...
    public void forks0() throws VerificationException {
        OutputValidator validator = unpack().forkCount(0).executeTest();
    ...
    public void forks0Redirected() throws VerificationException {
        OutputValidator validator = unpack().forkCount(0).redirectToFile(true).executeTest();
