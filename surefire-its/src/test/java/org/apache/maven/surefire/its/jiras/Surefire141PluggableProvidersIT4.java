 ...
 */
public class Surefire141PluggableProvidersIT4 extends SurefireJUnit4IntegrationTestCase {
    @BeforeClass
    public static void installProvider() {
        unpack(Surefire141PluggableProvidersIT4.class, "surefire-141-pluggableproviders-provider", "prov")
                .executeInstall();
    }
    ...
    public void invokeRuntimeException() {
        ...
        assertThat(hasErrorInLog && verifiedInLog)
                .describedAs("'" + errorText + "' could not be verified in log.txt nor *.dump file. (" + hasErrorInLog
                        + ", " + verifiedInLog + ")")
                .isTrue();
    }
    ...
    private static void assertErrorMessage(OutputValidator validator, String message) {
        ...
        for (String dump : dumpFiles) {
            validator.getSurefireReportsFile(dump).assertContainsText(message);
        }
    }
