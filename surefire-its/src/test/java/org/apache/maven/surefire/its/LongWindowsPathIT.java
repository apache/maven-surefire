...
public class LongWindowsPathIT extends SurefireJUnit4IntegrationTestCase {
    ...
    public void shouldRunInSystemTmp() throws Exception {
        ...
        OutputValidator validator =
                unpack().setForkJvm().showErrorStackTraces().executeTest().verifyErrorFreeLog();

        validator
                .assertThatLogLine(containsString("SUREFIRE-1400 user.dir="), is(1))
        ...
        for (String line : validator.loadLogLines()) {
            if (line.contains("SUREFIRE-1400 user.dir=")) {
                ...
                assertThat(line).contains(itBaseDir.getAbsolutePath());
            } else if (line.contains("SUREFIRE-1400 surefire.real.class.path=")) {
                assertThat(line).contains(new File(System.getProperty("java.io.tmpdir")).getCanonicalPath());
            }
        }
    }
