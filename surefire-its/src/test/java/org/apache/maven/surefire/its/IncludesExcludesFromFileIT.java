 ...
 */
public class IncludesExcludesFromFileIT extends SurefireJUnit4IntegrationTestCase {
    ...
    private void testWithProfile(String profile) {
        final OutputValidator outputValidator =
                unpack().activateProfile(profile).executeTest().verifyErrorFree(2);
    ...
    private void expectBuildFailure(final String profile, final String... messages) {
        final OutputValidator outputValidator =
                unpack().activateProfile(profile).maven().withFailure().executeTest();
