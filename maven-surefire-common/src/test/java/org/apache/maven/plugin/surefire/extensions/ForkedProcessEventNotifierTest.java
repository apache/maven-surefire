...
public class ForkedProcessEventNotifierTest {
    ...
    public static class DecoderOperationsTest {
        ...
        public void shouldSendByeEvent() throws Exception {
            ...
            assertThat(read).isEqualTo(":maven-surefire-event:\u0003:bye:");
        ...
        public void shouldSendStopOnNextTestEvent() throws Exception {
            ...
            assertThat(read).isEqualTo(":maven-surefire-event:\u0011:stop-on-next-test:");
            ...
            assertThat(logger.isCalled()).isFalse();
            assertThat(arguments.dumpStreamText).isEmpty();
            assertThat(arguments.isCalled()).isFalse();
            assertThat(listener.called.get()).isTrue();
        }
        ...
        public void shouldSendNextTestEvent() throws Exception {
            ...
            assertThat(read).isEqualTo(":maven-surefire-event:\u0009:next-test:");
        ...
        public void testDebug() throws Exception {
            ...
            assertThat(listener.msg).isEqualTo("msg");
        }
        ...
        public void shouldHandleErrorAfterUnknownOperation() throws Exception {
            ...
            assertThat(arguments.isCalled()).isTrue();

            assertThat(logger.isCalled()).isTrue();

            assertThat(logger.debug).hasSize(1);

            assertThat(logger.debug.peek()).contains(":maven-surefire-event:\u000c:abnormal-run:-:");
            ...
            assertThat(arguments.logWarningAtEnd.peek()).startsWith(dump);
        }
    ...
    public static class ReportEntryTest {
        ...
        public void testReportEntryOperations(
            ...
            EventChannelEncoder.class
                    .getMethod(operation[0], reportType, boolean.class)
                    .invoke(encoder, reportEntry, trim);

            ForkedProcessEventNotifier notifier = new ForkedProcessEventNotifier();

            ForkedProcessEventNotifier.class
                    .getMethod(operation[1], ForkedProcessReportEventListener.class)
    ...
    private static class StringEventAssertionListener implements ForkedProcessStringEventListener {
        ...
        public void handle(String msg) {
            called.set(true);
            assertThat(msg).isEqualTo(this.msg);
        }
    }
    ...
    private static class StackTraceEventListener implements ForkedProcessStackTraceEventListener {
        ...
        public void handle(@Nonnull StackTraceWriter stackTrace) {
            ...
            assertThat(stackTrace.getThrowable().getMessage()).isEqualTo(msg);

            assertThat(stackTrace.smartTrimmedStackTrace()).isEqualTo(smartStackTrace);

            assertThat(stackTrace.writeTraceToString()).isEqualTo(this.stackTrace);
        }
    }
    ...
    private static class StandardOutErrEventAssertionListener implements ForkedProcessStandardOutErrEventListener {
        ...
        public void handle(String output, boolean newLine, RunMode runMode, Long testRunId, String stack) {
            ...
            assertThat(testRunId).isEqualTo(this.testRunId);

            assertThat(output).isEqualTo(this.output);

            assertThat(newLine).isEqualTo(this.newLine);
        }
    }
