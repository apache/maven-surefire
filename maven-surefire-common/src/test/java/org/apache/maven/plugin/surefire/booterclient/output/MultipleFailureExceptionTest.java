...
public class MultipleFailureExceptionTest {
    ...
    public void test() {
        ...
        assertThat(e.getMessage()).contains("field is null").contains("read error");

        assertThat(e.getLocalizedMessage()).contains("field is null").contains("read error");

        assertThat(e.getSuppressed()).hasSize(2);

        assertThat(e.getSuppressed()).contains(suppressed1, suppressed2);

        assertThat(e.hasNestedExceptions()).isTrue();
    }
}
