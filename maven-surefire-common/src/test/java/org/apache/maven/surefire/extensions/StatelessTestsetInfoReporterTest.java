...
public class StatelessTestsetInfoReporterTest {
    ...
    public void shouldCloneReporter() {
        ...
        assertThat(clone).isInstanceOf(SurefireStatelessTestsetInfoReporter.class);
        assertThat(clone.toString()).isEqualTo("SurefireStatelessTestsetInfoReporter{disable=true}");
        assertThat(((SurefireStatelessTestsetInfoReporter) clone).isDisable()).isTrue();
    }

    @Test
    public void shouldAssertToStringReporter() {
        SurefireStatelessTestsetInfoReporter extension = new SurefireStatelessTestsetInfoReporter();
        assertThat(extension.toString()).isEqualTo("SurefireStatelessTestsetInfoReporter{disable=false}");
    }
    ...
    public void shouldCreateFileReporterListener() {
        ...
        assertThat(listener).isInstanceOf(FileReporter.class);
        assertThat(listener.getReportsDirectory()).isSameAs(reportsDirectory);
        assertThat(listener.getReportNameSuffix()).isSameAs(reportNameSuffix);
        assertThat(listener.getEncoding()).isSameAs(encoding);
        assertThat((boolean) getInternalState(listener, "usePhrasedFileName")).isEqualTo(false);
    ...
    public void shouldCreateConsoleReporterListener() {
        ...
        assertThat(listener).isInstanceOf(ConsoleReporter.class);
        assertThat(listener.getConsoleLogger()).isSameAs(consoleLogger);
        assertThat((boolean) getInternalState(listener, "usePhrasedClassNameInRunning"))
                .isEqualTo(false);
        assertThat((boolean) getInternalState(listener, "usePhrasedClassNameInTestCaseSummary"))
                .isEqualTo(false);
    ...
    public void shouldCloneJUnit5Reporter() {
        ...
        Object clone = extension.clone(classLoader);
        assertThat(clone).isNotSameAs(extension);
        assertThat(clone).isInstanceOf(JUnit5StatelessTestsetInfoReporter.class);
        ...
        assertThat(((JUnit5StatelessTestsetInfoReporter) clone).isDisable()).isTrue();
    ...
    public void shouldReportTestsetLifecycle() {
        ...
        assertThat(logs.getAllValues()).hasSize(1).contains("Running pkg.MyTest");
