...
public class SPITest {
    ...
    public void shouldNotLoadSpiDoesNotExist() throws IOException {
        assertThat(spi.lookup(NoServiceInterface.class, ctx)).isEmpty();

        assertThat(spi.load(NoServiceInterface.class, ctx)).isEmpty();
    }

    @Test
    public void shouldNotLoadEmptySpi() throws IOException {
        assertThat(spi.lookup(EmptyServiceInterface.class, ctx)).isEmpty();

        assertThat(spi.load(EmptyServiceInterface.class, ctx)).isEmpty();
    }

    @Test
    public void shouldLoad2SpiObjects() throws IOException {
        assertThat(spi.lookup(ExistingServiceInterface.class, ctx)).hasSize(2);

        assertThat(spi.lookup(ExistingServiceInterface.class, ctx))
                .containsOnly(SPImpl1.class.getName(), SPImpl2.class.getName());

        assertThat(spi.load(ExistingServiceInterface.class, ctx)).hasSize(2);

        assertThat(spi.load(ExistingServiceInterface.class, ctx)).contains(new SPImpl1(), new SPImpl2());
    }
}
