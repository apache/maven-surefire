     ...
     implements StreamConsumer
{
    ...
    private final class StdOutListener implements ForkedProcessBinaryEventListener
    {
        ...
        { //todo new String( binary, encoding ) v inom PR
            writeTestOutput( binary, true );
        }
    }
    ...
    private void writeTestOutput( byte[] binary, boolean isStdout )
    {
        getOrCreateConsoleOutputReceiver()
                .writeTestOutput( binary, 0, binary.length, isStdout );
        // todo zmen v inom PR na kodovanie podla sys prop ...encoding.reporting
    }
