...
public class ModularClasspathForkConfiguration extends DefaultForkConfiguration {
    ...
    private static File findModuleInfoPatchArgs(File patchFile) {
        // classic layout: patchFile == target/test-classes
        File argsFile = new File(patchFile, "META-INF/maven/module-info-patch.args");
        if (argsFile.isFile()) {
            return argsFile;
        }
        // module source hierarchy: patchFile == target/test-classes/<module>
    ...
    private static void appendModuleInfoPatchArgs(StringBuilder args, File patchArgs, String moduleName)
            throws IOException {
        // Files.newBufferedReader(Path) reads UTF-8, matching the compiler-written file
        try (BufferedReader reader = Files.newBufferedReader(patchArgs.toPath())) {
            String line;
