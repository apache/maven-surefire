pipeline {
    ...
    stages {
        stage('Unix Build') {
            ...
            steps {
                sh 'mvn clean install jacoco:report -B -U -e -fae -V -Prun-its,jenkins -Dsurefire.useFile=false -Dfailsafe.useFile=false -Dintegration-test-port=8084 "-Djdk.home=env.JDK_9_B181_HOME"'
        ...
        stage('Windows Build') {
            ...
            steps {
                bat 'mvn clean install jacoco:report -B -U -e -fae -V -Prun-its,jenkins -Dsurefire.useFile=false -Dfailsafe.useFile=false -Dintegration-test-port=8084 "-Djdk.home=env.JDK_9_B181_HOME"'
                jacoco changeBuildStatus: false, execPattern: '**/*.exec'
            }
            post {
                always {
