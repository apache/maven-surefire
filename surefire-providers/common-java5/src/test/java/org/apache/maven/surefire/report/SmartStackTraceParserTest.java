    ...
    extends TestCase
{
    ...
    public void testMultiLineMessageHandling()
    {
        ATestClass aTestClass = new ATestClass();
        try
        {
            aTestClass.aMultiLineTestErrorMessage();
        }
        catch ( RuntimeException e )
        {
            SmartStackTraceParser smartStackTraceParser =
                    new SmartStackTraceParser( ATestClass.class.getName(), e, null );
            String res = smartStackTraceParser.getString();
            assertEquals( "ATestClass.aMultiLineTestErrorMessage:69 Runtime " + "Only line 1 survives.",
                    res );
        }
    }
